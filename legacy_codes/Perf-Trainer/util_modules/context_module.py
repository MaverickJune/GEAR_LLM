...
from graph.learners_pro import dqn_pro_nx   

import flask
from flask import Blueprint

context_api = Blueprint("context_api", __name__)
...
@context_api.route('/con/test_action', methods=['POST'])
def test_action():
...
@context_api.route('/con/init_model', methods=['POST'])
def init_model():
    init_params = json.loads(flask.request.json)
    m_type = init_params["m_type"]
    learner = learner_factory(m_type)
...
@context_api.route('/con/rm_model', methods=['GET'])
def rm_model():
    status = rm_client(flask.request.args.get("m_id"))
    return flask.jsonify({"status":status})

@context_api.route('/con/clear_all')
def clear_all():
    del_list = []
...
@context_api.route('/con/get_action', methods=['POST'])
def get_action():   
...
@context_api.route('/con/get_action_test', methods=['POST'])
def get_action_test():   
    params = json.loads(flask.request.json)
    m_id, data = params["m_id"], params["data"]
    # ,params["s"]
...
@context_api.route('/con/get_test_power', methods=['POST'])
def get_test_power():   
    params = json.loads(flask.request.json)
    m_id = params["m_id"]
...
@context_api.route('/con/train', methods=['POST'])
...
@context_api.route('/con/check_model_status', methods=['GET'])
...
@context_api.route('/con/request_update', methods=['GET'])
def request_update():
    client_id = check_client(flask.request.args.get("m_id"))
    if client_id:
        learner = globals()["DeepDep"][client_id]
...
@context_api.route('/con/train_context', methods=['POST'])
def train_context():
    """Train context model (for dqn_pro_nx)"""
    params = json.loads(flask.request.json)
    m_id = params["m_id"]
    ...
    if client_id:
        learner = globals()["DeepDep"][client_id]
        learner['pipe'].send({"cmd":"CONTEXT"})
        
        # Wait for response
        response = learner['pipe'].recv()
        return flask.jsonify(response)
    return flask.jsonify({"status":False, "error":"Client not found"})

@context_api.route('/con/train_rl', methods=['POST'])
def train_rl():
    """Train RL controller (for dqn_pro_nx)"""
    params = json.loads(flask.request.json)
    m_id = params["m_id"]
...
def learner_factory(m_type):
    """
    Factory function to get learner based on model type
    
    Available learners:
    - "dqn_nx": DQN for Jetson CPU DVFS (CPU-only)
    - "dqn_pro_nx": Advanced DQN with RNN context for Jetson
    - "MOCK": Mock learner for testing
    """
    learner_dict = {
        "MOCK": mock_learner,
        "DQN": dqn_nx,
        "dqn_nx": dqn_nx,
        "dqn_pro_nx": dqn_pro_nx
    }
