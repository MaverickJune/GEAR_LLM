...
def run_monitoring(stop_queue, data_queue, interval=0.1):
    """
    Separate process function to run DVFS monitoring
    Continuously monitors power and temperature until stop signal received
    
    Args:
        stop_queue: multiprocessing.Queue to receive stop signal
        data_queue: multiprocessing.Queue to send monitoring data back to main process
        interval: Sampling interval in seconds
    """
    print("Monitoring process started")
    
    # Create monitor instance in this process
    monitor = Monitor()
...
def main():
    ...
    monitoring_stop_queue = Queue()
    monitoring_data_queue = Queue()  # New queue for monitoring data
    ...
    mon_process = Process(
        ...
        args=(monitoring_stop_queue, monitoring_data_queue, 0.1)
    )
    ...
    # Stop monitoring
    monitoring_stop_queue.put(True)
    mon_process.join()
    ...
    if not monitoring_data_queue.empty():
        monitoring_data = monitoring_data_queue.get()
    ...
    if monitoring_data:
        log_power = monitoring_data.get('log_power', [])
        log_temp = monitoring_data.get('log_temp', [])
        ...
        print(f"Power samples collected: 0")
        print(f"Temperature samples collected: 0")
